...
const Section = ({id,priceHome,address,MetroHome,roomHome,WhoCanTake,measureHome,Items,dateTime,deleteBasket}) => {
    ...
    const newData = [
      [keepingImgSource,id, priceHome, address, MetroHome, roomHome, WhoCanTake, measureHome, Items, dateTime],
    ];
    ...
    const SendBasket = () => {
      ...
      localStorage.setItem('Section', JSON.stringify([...LastinfoLocal, ...newData]));
        ...
        const indexToRemove = updatedData.findIndex(item => item[1] === id);
     ...
     useEffect(() => {
      const isHave = (JSON.parse(localStorage.getItem("Section")) || []).some((x) => (x[1] == id));
      console.log(isHave);
      if (isHave) {
        setchangeColor(true);
        setCheckRptrData(false);
