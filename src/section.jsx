...
const Section = ({
  ...
  useEffect(() => {
    const timer = setTimeout(() => setShowLoad(true), 5000);

    return () => clearTimeout(timer);
  }, [keepingImgSource.length]);
  return (
      ...
      <div className=" p-2 mt-4">
        <div className="card-home">
          ...
          <div className="pb-2">
            <Link to={`/Kart/${id}`}>
              <p>
                Qiymet:<span>{priceHome}</span>
                <span>{price}</span>
              </p>
              <p>
                Ünvan:<span>{cutString(address, 20)}</span>
              </p>
              <p>
                Metro:<span>{MetroHome}</span>
              </p>
              <p>
                Otaq sayi:<span>{roomHome}</span>
              </p>
              <p>
                Rayon:<span>{region}</span>
              </p>
              <p>
                Sahe:
                <span>
              ...
              <p>
                Əşya:<span>{Items}</span>
              </p>
              <p>
                Tarix:<span>{dateTime}</span>
              </p>
            </Link>
          </div>
        </div>
