...
import SectionCustomer from "./LandSectionCustomer";
...
const LandCustomer = () => {
  ...
  const SearchNumber = useSelector((state) => state.LandAdmin.SearchNumber);
  const SearchRedux = useSelector((state) => state.LandAdmin.SearchModel);
    ...
    <div>
      <div>
        ...
        <div className="d-flex flex-wrap">
          {parsedData.map((x) => (
            <SectionCustomer props={x} data={x} />
          ))}
          {parsedData.length === 0 && (
      ...
      <Pagenation
        countOfPagination={totalPages}
        setPage={handlePageChange}
        current={currentPageR}
      />
    </div>
  );
};

export default LandCustomer;
