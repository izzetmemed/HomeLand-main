...
export const fetchData = createAsyncThunk(
  'OfficeAdmin/fetchData',
  async ({ page ,SearchModel,Number}) => {
    var response;
    var stringNumber=String(Number);
    switch (SearchModel) {
      case 'id':
        response = await FetchGetAll(`Office/AdminId-${Number}`);
        return response.data;
      case 'AdminONumber':    
        response = await FetchGetAll(`Office/AdminONumber-${stringNumber}`);
        return response.data;
      case 'AdminCNumber':
        response = await FetchGetAll(`Office/AdminCNumber-${stringNumber}`);
        return response.data;
      default:
        response = await FetchGetAll(`Office/Normal-${page}`);
        return response.data;
    }
  }
);
